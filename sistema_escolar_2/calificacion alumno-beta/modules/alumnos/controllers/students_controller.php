<?php  
	...
	if($login -> session_verify()){
		...
		if(!$login -> is_admin($userType)){
            ...
            $groupOptions = $group -> get_selectOptions();

            $groupOptions = $groupOptions['data'];
           
            require_once('modules/profesores/models/Teacher.php');

			$teacher = new Teacher();

            $teacherOptions = $teacher -> get_selectOptions();

            $teacherOptions  = $teacherOptions['data'];         

			require_once("modules/alumnos/views/theme/students_view.php");
