class Level{
	...
	search_data(){
		...
		let dataJson = 'request='+JSON.stringify({request:'search-level',
						data:dataForm});
		...
		this.ajax.send_data({method:'post', 
							functionName:function(data){
								...
								if(data.status == 'done'){
									...
									let levelTable = objectSelf.levelTable;

									levelTable.set_table(data.data);

									//cargar la tabla con los resultados
									colegio.innerHTML(objectSelf.tableContainer, levelTable.get_levels_table());
									
									//botones de la tabla de resultados 

									levelTable.status_btn();//boton para cambiar estado
									
									levelTable.update_dataBtn((e) => {

										objectSelf.open_set_form(e);

									});//boton para editar informacin
									
									levelTable.del_btn((e) => {
						   		
						   				objectSelf.del_data(e);

									});//boton para borrar grado
		...
		objectSelf.ajax.send_data({method:'post', 
						    functionName:function(data){
						    	...
						    	if(data.status == 	'done'){
										...
										datasignature[0] = getData.gr_nombre;
										datasignature[1] = getData.gr_estado;
										datasignature[2] = id;																	
		...
		objectSelf.ajax.send_data({method:'post', 
						    functionName:function(data){
								...
								if(data.status == 'done'){
									...
									let newData = levelTable.table.remove_row(id);

									levelTable.table.set_bodyTable(newData);
